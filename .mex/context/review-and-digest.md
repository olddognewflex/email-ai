...
- **`apps/tui`** — an Ink/React keyboard TUI launched by the `eai` shim, which defaults to
  ...
  `GET /sender-rules/:id`: those rows stay linked and unchanged. When the save changed
  pattern, match type or category, or switched the rule to `classify`, the message also
  offers `C`. The pure parts live in `rule-edit.ts`. `C` (`ReclassifyScreen`) re-runs the
  selected rule over mail that is already classified through
  `POST /sender-rules/:id/reclassify`. It opens with a dry run for `scope=linked`; `s`
  toggles scope and `m` the release mode, each with a new dry run. `reclassifyRule()`
  hardcodes `dryRun=true`; `applyReclassify()` is the TUI's only live call and runs only on
  `y` after the dry run for the current options has loaded, with a second `y` when the dry
  run expects AI calls (it shows the provider and estimated cost). `U` undoes the batch the
  screen applied, after a y/n, via `POST /sender-rules/reclassify-batches/:batchId/undo`.
  The pure parts (gate, cost message, count lines, sample rows budgeted to the width) live in
  `reclassify-view.ts`. `G` (`SuggestionsScreen`) reads `/sender-rules/suggestions`. It creates a
...
- `com.odnf.email-ai.hourly-sync` — the `sync` stage every hour on the hour: ingest, classify,
  then apply `trash` sender rules. The apply step reads `GET /mailbox-actions/status` and moves
  mail only when it reports `writesEnabled: true`; otherwise it runs a dry run and logs the
  would-move totals. Its failure is isolated like any other step.
