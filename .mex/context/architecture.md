...
- **No authentication or authorization.** Every endpoint is open. Three local-only
  protections stand in for it:
  - the API binds to 127.0.0.1 (`EMAIL_AI_HOST` overrides it);
  - requests with a foreign `Host` header get a 403 (DNS rebinding);
  - every non-GET request needs an `X-Email-AI-Client` header (global `APP_GUARD`),
    which is a CSRF guard rather than auth.

  Do not add features that assume a current user, and send the header from any new
  client.
...
- **No mailbox writes except one audited MOVE.** The system never deletes, flags, or replies
  to mail. The single exception is `MailboxWriterService` (`modules/mailbox-actions/`): an
  IMAP `UID MOVE` of INBOX mail matched by an enabled `trash` sender rule into the
  server-advertised `\Trash`, and the reverse move for undo. It is off unless
  `MAILBOX_WRITES_ENABLED=true`, refuses servers without `MOVE` (imapflow would otherwise
  emulate it with an expunge), and writes a `MailboxAction` row before and after every move.
  See the non-negotiables in `.mex/AGENTS.md`.
