...
- Nothing may write to a mailbox, and no endpoint may assume an authenticated user — there is
  no auth layer at all. **Exception:** `MailboxWriterService` (`modules/mailbox-actions/`) is
  the single, kill-switched, audited write path (IMAP MOVE to `\Trash` and back). A new
  module that needs a mailbox write calls it; it never builds its own IMAP write. Get IMAP
  clients from the `IMAP_CLIENT_FACTORY` token, not `new ImapFlow`.
...
- [ ] No secret is logged or returned; nothing mutates a mailbox outside `MailboxWriterService`.
